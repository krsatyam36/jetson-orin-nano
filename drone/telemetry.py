...
def read_telemetry():
    # Target port from your successful hardware handshake on J2
    port = '/dev/ttyACM0'
    baud = 115200
    ...
    try:
        ...
        # --- THE FIX: This loop must be indented inside the try block ---
        while True:
            msg = master.recv_match(type=['ATTITUDE', 'VFR_HUD', 'GPS_RAW_INT'], blocking=True, timeout=1.0)
            ...
            if msg is not None:
                ...
                # Speed and Altitude Details
                    ...
                    print(f"[HUD] Alt: {msg.alt:.1f}m | Heading: {msg.heading}° | Throttle: {msg.throttle}%")
                
                # GPS Connectivity
                elif msg_type == 'GPS_RAW_INT':
                    print(f"[GPS] Satellites: {msg.satellites_visible} | Fix Type: {msg.fix_type}")
